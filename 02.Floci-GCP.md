...
gcloud config set api_endpoint_overrides/container http://localhost:4588/
```
...
curl -sS \
  ...
  "http://localhost:4588/v1/projects/floci-local/locations/-/clusters" | jq
```
...
_Test from Floci's create API directly:_
```
curl -i -X POST \
  "http://localhost:4588/v1/projects/floci-local/locations/us-central1-a/clusters?clusterId=dev-cluster" \
  -H "Authorization: Bearer fake-token" \
...
_Verify:_
```
curl -i http://localhost:4588/v1/projects/floci-local/locations/us-central1-a/clusters
```
...
curl -sS \
  -H "Authorization: Bearer fake-token" \
  "http://localhost:4588/v1/projects/floci-local/locations/us-central1-a/clusters/dev-cluster" |
